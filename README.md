...
システム辞書は `.deb` に同梱されている（`/usr/share/karukan-im/dict.bin`）ため、
別途の導入は不要です。自分で用意した辞書に差し替える方法や、読み込みの優先順位は
[辞書](docs/dictionary.md)を参照してください。
...
- **バージョン表記**: 動作中のビルドがログと初回ロード表示に出る。リリースタグ上のビルドはタグ名(`v0.1.0-rakukan.2`)、タグ間の開発ビルドは最終リリースからの距離付き(`v0.1.0-rakukan.2-3-g1234567`)
...
- **日付・時刻の変換**: 「きょう」→ `2026/09/22`、「いま」→ `15:30`、「にちじ」→ `2026/09/22 15:30` など。フレーズと出力形式は `[date]` で登録できる（[詳細](docs/date.md)）
- **候補ソースの絞り込み**: 変換中に `Ctrl+T` / `Ctrl+R` で候補を出どころ別（📝学習 → 📚辞書 → 🤖AI → 🔄記号変換）に切り替え。`Ctrl+I` で🤖AI候補へ直行
- **候補ウィンドウの表示切替**: `[display] candidate_window = "conversion"` にすると、入力中は候補ウィンドウを出さず Space で変換を始めてから開く

> [!TIP]
> **日付候補は `Ctrl+R` で呼び出します。** 候補一覧では日付（📅）が後方に並び、システム辞書の予測候補が先に入るため、読みによっては Space 送りで実質たどり着けません（`きょう` は候補が 35,000 件を超えます）。`きょう` → `Space` → `Ctrl+R` と押すと🔄ビューが開いて日付が先頭に並ぶので、`Ctrl+1`〜`9` で選べます（候補選択は素の数字ではなく **Ctrl+数字** です）。
...
- [日付・時刻](docs/date.md) — きょう/いま などの日付・時刻変換、フレーズと出力形式の登録、`Ctrl+R` での呼び出し
...
正式なリリース作成時は、`develop` をコミット・push し、`origin/develop` と一致する未変更の状態で `scripts/github-release.sh` を使用してください。
...
ブランチ運用: `main` は上流 `togatoga/karukan` の同期専用とし、独自変更は入れません。派生版の開発・リリースは `develop` で行います。機能はフェーズ単位の feature ブランチ(`feat/rakukan-*`)で実装し、PR で `develop` へ取り込みます。上流更新は `main` から `develop` へ merge し、機能移植と同じコミットに混ぜません。リリースタグは `develop` のコミットに付けます。
