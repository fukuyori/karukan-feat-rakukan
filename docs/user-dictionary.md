...
- **ファイルの追加・編集・削除は IME を再起動せずに反映されます**（数秒おきに更新時刻とサイズの変化を確認し、変わったファイルだけ再読込。あとからディレクトリを作った場合も検出されます）
- 読み込みに失敗したファイル（壊れたバイナリ等）は、直前に正常に読めた内容がそのまま使われ続けます。ファイルを修正すると再度読み込まれます
