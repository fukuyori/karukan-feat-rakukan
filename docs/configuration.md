...
stale_days = 0                 # この日数使わなかった学習を読込時に削除（0 = 無効）
...
既定で以下の6モデルが定義済みです（ユーザーの `config.toml` の `[models]` はキー単位でマージされ、同じキーは上書き、既定のエントリはそのまま残ります）。
...
F16 は `model` / `light_model` で指定したときだけダウンロードされます（`--prefetch-models` は設定が参照するモデルだけを温めます）。
...
- 記録単位は「読み全体 → 確定した文字列全体」の1エントリ。chunk・文節ごとの部分学習は行わない（文中の個々の単語は個別には学習されない）
- 学習されるのは、候補ウィンドウで自分が選んだ確定（学習・辞書・モデル・記号変換の候補）と、かな・カタカナの確定。**ライブ変換をそのまま Enter で確定した場合（候補ウィンドウなし）と、変換が得られなかったときの読みフォールバック候補は学習されない** — 誤った第一候補が自己強化されるのを防ぐため
- **確定した文字列が50文字（`max_surface_chars`）を超える場合、その確定は一切学習されない**。50文字以下でも一文まるごとのエントリは同じ読みを打ち直したときにしかヒットしないため、長文をまとめて確定する使い方では学習が効きにくい。単語・文節単位でこまめに確定すると学習がよく効く
...
- `stale_days` を設定すると、最後に選択してからその日数を超えたエントリが起動時（キャッシュ読込時）に削除される。`0`（既定）で無効。削除は次回保存時に `learning.tsv` へ反映される
