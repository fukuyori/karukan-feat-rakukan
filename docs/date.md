...
候補は候補一覧の後方（📅）に並びます。確定しても学習履歴には記録されません。日付は明日には古くなるので、記録すると「きょう」の変換に昨日の日付が出続けてしまうためです。

## 呼び出し方（`Ctrl+R`）

変換中に **`Ctrl+R` を1回**押すと🔄変換ビュー（幅・記号などの変換候補と同居）が開き、日付が**先頭**に並びます。`Ctrl+1`〜`9` で選んで確定します（候補選択は素の数字ではなく Ctrl+数字です）。

```
きょう → Space → Ctrl+R → Ctrl+1
```
...
> [!IMPORTANT]
> 候補一覧（Space）を送って日付にたどり着くのは現実的ではありません。システム辞書の予測候補（読みの前方一致）が日付より前に入り、その件数に上限がないためです。実測では `きょう` の候補は 35,517 件で、日付はその 35,510 番目以降（3,947 ページ中の最終ページ）でした。`Ctrl+R` を使ってください。

`Ctrl+T` でも同じビューに届きますが、📝学習 → 📚辞書 → 🤖AI → 🔄記号変換 の順に回るので4回押す必要があります。`Ctrl+R` は逆回りで、1回で🔄に着きます。全体の一覧に戻るには `Esc` → `Space` です。
